...
export default function NutritionLocker() {
  ...
  async function compileProtocol() {
    ...
    const plan = buildPlanPayload({ dietStyleId: dietStyle, energy, band });
    try {
      const res = await assignNutrition(scholarId, {
        plan, tdee_target: energy, diet_style: dietStyle, allergens: allergy, source: 'scheduler',
      });
  ...
  async function pushOversight() {
    ...
    const plan = buildPlanPayload({ dietStyleId: dietStyle, energy: calorieOverride, band });
    try {
      await assignNutrition(scholarId, {
        plan, tdee_target: calorieOverride, diet_style: dietStyle, allergens: allergy,
        phase, directive, source: 'oversight',
      });
      if (!mounted.current) return;
      ...
      <header className="nl-hero">
        ...
        <div className="nl-regime">
          <div className="nl-regime-col nl-regime-col--grow">
            ...
            <span className="nl-regime-val">{dietStyleLabel(dietStyle)}{allergy !== 'None' ? <em> · {allergy}</em> : null}</span>
          </div>
