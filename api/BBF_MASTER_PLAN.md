...
## [x] 4.3h · Friction Scanner + Sovereign Linguistics React surfaces · live wire to bbf-agentic-prehab + bbf-agentic-linguist + X-BBF-Admin-Token forwarding · commit `08b6524` · 2026-05-26 · Phase 4.3 Stage 2 surface gap-fill · operator's "Red Team Bug Fix: Live Video Audit Failures"
- **Why:** The operator ran a physical mobile QA audit with video evidence after §4.3g and found two severed features the Vault React SPA was MISSING entirely. Both exist in the legacy bbf-app.html (Friction Scanner at line 17246, Linguist Translator at line 1462) but neither had been ported to the Vault React tree, so the operator's audit found "dead state bindings" where there were no bindings at all. Plus a third real fault the operator's framing surfaced: the `callEdgeFunction` helper never forwarded `X-BBF-Admin-Token`, so any `bbf-agentic-*` edge function deployed with `BBF_COACH_AGENT_TOKEN` env-set returned 401 for the coach path (silent admin-side auth gap).
- **How (this session · 5-file surface · 3-layer fix):**
  1. **Data layer · `vault/src/services/supabaseClient.ts`** · `callEdgeFunction(name, body, opts?)` gains optional `headers?: Record<string,string>` on `opts` · merges into the base apikey + Authorization + Content-Type triplet · zero impact on existing call sites (opts defaults `{}`) · new private `_agentHeaders()` helper returns `{'X-BBF-Admin-Token': <token>}` when `getCoachAgentToken()` returns one, empty object otherwise (coaches forward the header · athletes get the no-header path that the function defaults to allow when env unset). Two new public helpers: `generatePrehabMatrix(uidSlug, payload?)` POSTs `/functions/v1/bbf-agentic-prehab` and returns `{matrix: PrehabMovement[]}` where each movement is `{name, duration, focus, reason}` · pre-resolves `actual_uuid` via the local resolver so the edge function can skip its own RPC lookup on the warm-cache path · null forwarded if the local resolver doesn't have the slug. `translateCoachingCue(uidSlug, englishCue, targetLanguage?)` POSTs `/functions/v1/bbf-agentic-linguist` and returns `{translation, phonetic, literal_meaning}` · defaults to `es` (Spanish). Both forward the admin-token header via `_agentHeaders()`.
  2. **Friction Scanner UI · `vault/src/components/PrehabReadiness.tsx`** + **`PrehabReadiness.module.css`** · new state `friction: string` (textarea, 600-char cap), `matrix: PrehabMovement[] | null`, `generating: boolean` + `generatingRef: useRef(false)` (synchronous shield · mirrors the §4.3g pattern from the same component's existing `submittingRef`). New handler `handleGenerateMatrix` · refs first then setState · resolves `getActiveUid()` → null surfaces "No active session" banner · trims + length-caps friction text · calls `generatePrehabMatrix` · on success sets matrix; on failure surfaces error string. New JSX appended after the existing readiness submit · scannerHeader (kicker+title+sub) · frictionField (textarea with strict containment `width:100%; max-width:100%; min-width:0; box-sizing:border-box` + char counter footer) · submit button `disabled={generating}` with label "Generate Recovery Matrix" → "Generating…" · `.matrixList` ul `repeat(auto-fit, minmax(min(100%, 14rem), 1fr))` 1-up on phones / 3-up on wide · each `.matrixCard` 4-row grid (index badge + name + duration on row 1 · focus row 2 · reason row 3) · clamp() typography on every text scale.
  3. **Sovereign Linguistics UI · `vault/src/components/ProfileSettings.tsx`** + **`ProfileSettings.module.css`** · new state `englishCue: string` (200-char cap), `targetLanguage: string` (defaults `es`), `translation: TranslationResponse | null`, `translating: boolean` + `translatingRef: useRef(false)`. `LANGUAGE_OPTIONS` constant `[es, pt, fr, de]` (matches the linguist edge function's `normalizeLanguage` fallthrough · `en` excluded · translating English → English is a no-op). New handler `handleTranslate` early-returns when `englishCue.trim()` is empty (avoids the doomed `missing_english_cue` 400) · calls `translateCoachingCue(uid, cue, language)` · same shield pattern. New JSX appended after the cloudPending honesty banner · linguistHeader · `.linguistRow` auto-fit grid `repeat(auto-fit, minmax(min(100%, 12rem), 1fr))` so cue input + language select sit side-by-side on wide / stacked on phones · submit button `disabled={translating}` with label "Translate to Gym-Floor Slang" → "Translating…" · `.translationCard` 3-row grid (Gym-floor phrase primary in brand-gold `clamp(1rem, 3.4vw, 1.25rem)` + Phonetic secondary + Literal meaning secondary · each row separated by dashed bottom-border).
  4. **Operator-directive compliance.** "Don't touch the CSS grids" honored verbatim · zero modifications to existing `.row` / `.fieldGrid` / `.macrosGrid` / `.activityGroup` / `.composite` / `.matrix` / `.metricStrip` rules. Every new section uses ONLY new CSS classes, all using the same `repeat(auto-fit, minmax(min(100%, …), 1fr))` + clamp() + max-width:100% containment patterns the operator's visual gauntlet already approved. `VaultShell.tsx` byte-identical · the new surfaces live inside existing tabs (Prehab gets Friction Scanner · Profile gets Linguistics) · no new tab IDs · the pre-mount + state-survives-tab-switch invariants from Phase 4.3b preserved.
- **Done when (this entry):** `npm run typecheck` zero errors across all 3 projects · `npm run build` zero warnings · 85 modules transformed · CSS chunk 21.90 → 26.22 kB (+4.32 kB · two new visual sections) · JS bundle 189.40 → 196.29 kB (+6.89 kB · two new state branches + handlers + helpers + headers-forwarding hook) · gzip 60.05 → 62.14 kB (+2.09 kB) · `VaultShell.tsx` byte-identical pre/post commit.
- **Shipped (this session · 5 files · +605 / −10 lines):**
  - `vault/src/services/supabaseClient.ts` (edit · `callEdgeFunction` opts.headers + `_agentHeaders` helper + `generatePrehabMatrix` + `translateCoachingCue` + `PrehabMovement` / `PrehabMatrixResponse` / `PrehabMatrixRequest` / `TranslationResponse` type exports)
  - `vault/src/components/PrehabReadiness.tsx` (edit · friction state + matrix state + generatingRef shield + handleGenerateMatrix + Friction Scanner JSX section)
  - `vault/src/components/PrehabReadiness.module.css` (edit · 15 new classes for the scanner + matrix cards)
  - `vault/src/components/ProfileSettings.tsx` (edit · cue + language + translation state + translatingRef shield + handleTranslate + Linguistics JSX section)
  - `vault/src/components/ProfileSettings.module.css` (edit · 11 new classes for the linguist + translation card)
- **Validation (this session):**
  - `npm run typecheck` (tsc -b --noEmit) · zero errors across `tsconfig.app` + `tsconfig.node` + `tsconfig.e2e`.
  - `npm run build` (tsc -b && vite build · vite 5.4.21) · zero warnings · 85 modules transformed · bundle 189.40 → 196.29 kB (+6.89 kB / +2.09 kB gzip) · CSS chunk 21.90 → 26.22 kB (+4.32 kB / +0.58 kB gzip) · built in 1.37 s.
  - Edge function response shapes verified UNCHANGED across the §6.0k Anthropic Proxy Sweep · `bbf-agentic-prehab` still returns `{matrix: [{name, duration, focus, reason}]}` · `bbf-agentic-linguist` still returns `{translation, phonetic, literal_meaning}`. The operator's "JSON output shape likely shifted" hypothesis was wrong · the actual fault was that the React surfaces didn't exist yet.
  - The §4.3g red-team resolver fix (`_ensureUidMap` clear-on-failure + lowercase-at-store + retry-on-miss) applies transitively to `generatePrehabMatrix` via its `resolveUserUuid(uidSlug)` pre-resolution call · the Friction Scanner inherits the `uid_not_resolvable` immunity from §4.3g without any new code.
  - The §4.3g synchronous-shield pattern (useRef-backed locks) extends from the existing `submittingRef` / `scanningRef` / `analyzingRef` to the new `generatingRef` (PrehabReadiness) + `translatingRef` (ProfileSettings) · 7 total synchronous shields across the SPA · every action button is now race-immune.
- **Out of scope (next CNS-intel React port):** The legacy bbf-app.html `BBF_PREHAB_INTEL` / `BBF_CNS_AGENT` snapshot + damper payload fields (`cns_snapshot` / `damper`) are NOT forwarded from the React layer · the Vault doesn't have those modules ported yet · the edge function gracefully handles their absence (uses defaults). Queued for the Phase 5+ CNS-intel React port. The shared `useSubmitLock()` custom hook is still queued (now 7 ref-shield instances across 6 components).
