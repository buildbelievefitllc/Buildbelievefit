// BBF LANGUAGE ENGINE — Build Believe Fit LLC
// window.BBF is the global object — buttons call BBF.setLang()
...
window.BBF = {
  ...
  setLang: function(lang) {
    ...
    // Update every button that has a data-lang attribute
    ...
    document.querySelectorAll('input[name="_next"]').forEach(function(input) {
      input.value = window.BBF.REDIRECTS[lang] || window.BBF.REDIRECTS.en;
    });
  ...
  toggleTrainingMode: function(active) {
    ...
    if (label) {
      label.textContent = active ? 'CUES:' + window.BBF.lang.toUpperCase() : 'MODE';
      label.style.color = active ? '#f5c800' : '#4a4570';
    }
    if (track) {
      track.style.background = active ? '#6a0dad' : '#1a1a1a';
      track.style.borderColor = active ? '#8b1abf' : '#2a2640';
    }
    if (thumb) {
      thumb.style.left = active ? '16px' : '2px';
      thumb.style.background = active ? '#f5c800' : '#4a4570';
    }
  },
  ...
  init: function() {
    ...
    if (isApp) {
      ...
      var savedTrain = false;
      try { savedTrain = localStorage.getItem('bbf_train') === 'true'; } catch(e) {}
      if (savedTrain) window.BBF.toggleTrainingMode(true);
    }
  }
