...
// Uses ONLY data-lang-key attributes. Clean. No ID fallback.
(function() {
  'use strict';
  ...
  var T = {
    ...
    'app-step2': { en:'Tap menu then "Add to Home Screen"', es:'Toca el menú y "Agregar a pantalla de inicio"', pt:'Toque no menu e "Adicionar à tela inicial"' },
...
// Form labels added after initial deployment
var FORM_T = {
...
// Merge form keys and re-run on DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
  var lang = localStorage.getItem('bbf_lang') || 'en';
  document.querySelectorAll('[data-lang-key]').forEach(function(el) {
    var key = el.getAttribute('data-lang-key');
    if (FORM_T[key] && FORM_T[key][lang]) el.innerHTML = FORM_T[key][lang];
  });
  if (window.BBF_LANG) {
    ...
    window.BBF_LANG.set = function(l) {
      ...
      document.querySelectorAll('[data-lang-key]').forEach(function(el) {
        var key = el.getAttribute('data-lang-key');
        if (FORM_T[key] && FORM_T[key][l]) el.innerHTML = FORM_T[key][l];
      });
    };
  }
});
