...
// ── PATCH: Additional translation keys ─────────────────────
// Added after initial deployment to cover remaining English sections
(function() {
  var EXTRA = {
    // Program names
  ...
  // Merge into main T object and re-apply if BBF_LANG is loaded
  if (window.BBF_LANG) {
    Object.keys(EXTRA).forEach(function(k) {
      var el = document.getElementById(k);
      var lang = window.BBF_LANG.get();
      if (el && EXTRA[k][lang]) el.innerHTML = EXTRA[k][lang];
    });

    // Patch setLang to also apply EXTRA keys
    var _orig = window.BBF_LANG.set;
    window.BBF_LANG.set = function(l) {
      _orig(l);
      Object.keys(EXTRA).forEach(function(k) {
        var el = document.getElementById(k);
        if (el && EXTRA[k][l]) el.innerHTML = EXTRA[k][l];
      });
    };
  }
})();

// ── PATCH 2: Final remaining English sections ───────────────
(function() {
  var PATCH2 = {
    // Transformation body paragraphs
    'trans-p1': {
    ...
    'contact-sub': {
      ...
      pt: 'Pronto para começar? Tem perguntas? Entre em contato diretamente — Akeem responde rápido.'
    },
    // Footer nav links
  ...
  function applyPatch(lang) {
    Object.keys(PATCH2).forEach(function(k) {
      var el = document.getElementById(k);
      if (el && PATCH2[k][lang]) el.innerHTML = PATCH2[k][lang];
    });
  }

  // Apply on load
  if (window.BBF_LANG) {
    applyPatch(window.BBF_LANG.get());
    var _prev = window.BBF_LANG.set;
    window.BBF_LANG.set = function(l) {
      _prev(l);
      applyPatch(l);
    };
