# AGENTS.md — Build Believe Fit (BBF)

Authoritative project brain for Codex sessions. This file consolidates the
...
## 4 · Central model router (REQUIRED for every Codex call)

All Codex-calling edge functions route through `supabase/functions/_shared/model-router.ts`.
...
|---|---|---|
| HAIKU | `Codex-haiku-4-5` | retries, narration, i18n, snapshots, forecasts (low-stakes) |
| SONNET | `Codex-sonnet-4-6` | vision/biomechanics, onboarding dialog, prehab (mid-complexity) |
| OPUS | `Codex-opus-4-8` | PAR-Q+, wellbeing/ED escalation, cardiac intercept (**safety-critical only**) |
...
> **`Codex-opus-4-8`**. `MODELS.OPUS` in `model-router.ts` is the single source of truth and
> is pinned to 4.8 — never inline a model string in a caller. The remaining `Codex-opus-4-7`
