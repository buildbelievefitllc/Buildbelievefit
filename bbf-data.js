...
var VAULT_MODULES = [
  {
    id: 1,
    title: 'The Human Performance Foundation',
    desc: 'Who this program is built for and why standard fitness advice fails high-demand professionals.',
    duration: '18 min',
    locked: false,
    tag: 'Mindset',
    tag_icon: '&#x1F9E0;',
...
// ─── DATA BRIDGE: CLINICAL YIELD LOG ──────────────────────────────
async function logBiometricYield(client_id, markerData) {
  var payload = {
    client_id: client_id,
    marker_01_cns_readiness: markerData[0],
    marker_02_grip_strength: markerData[1],
    marker_03_rpe_tolerance: markerData[2],
    marker_04_hrv: markerData[3],
    marker_05_sleep_efficiency: markerData[4],
    marker_06_joint_mobility: markerData[5],
    marker_07_hydration: markerData[6],
    marker_08_cortisol_trend: markerData[7],
    marker_09_muscle_soreness: markerData[8],
...
async function fetchClientYieldHistory(client_id) {
  ...
  return fetch(SUPA_URL + '/rest/v1/clinical_yield_log?client_id=eq.' + client_id + '&order=created_at.asc', {
    ...
    data.forEach(function(row) {
      history.push([
        row.marker_01_cns_readiness,
        row.marker_02_grip_strength,
        row.marker_03_rpe_tolerance,
        row.marker_04_hrv,
        row.marker_05_sleep_efficiency,
        row.marker_06_joint_mobility,
        row.marker_07_hydration,
        row.marker_08_cortisol_trend,
        row.marker_09_muscle_soreness,
        row.marker_10_resting_hr,
        row.marker_11_blood_pressure
      ]);
    });
    return history;
  });
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { logBiometricYield: logBiometricYield, fetchClientYieldHistory: fetchClientYieldHistory };
}
