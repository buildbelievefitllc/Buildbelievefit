function _global_esc(s){ return String(s == null ? '' : s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
// ═══════════════════════════════════════════════════════════════════
...
function RH() {
  ...
  if (notes.length) {
    ...
    document.getElementById('cnlist').innerHTML = notes.map(function(n) {
      return '<div class="cn2"><div class="cnh"><span>&#x1F4CB;</span><span class="cnl">Coach Note</span><span class="cnd">' + FMT(n.date) + '</span></div><div class="cnt">' + _global_esc(n.notes) + '</div></div>';
    }).join('');
...
function REL() {
  document.getElementById('exl').innerHTML = EX.map(function(e, i) {
    return '<div class="exi"><span>' + _global_esc(e) + '</span><button class="exd" data-i="' + i + '">&#x2715;</button></div>';
  }).join('');
...
function RP() {
  ...
  document.getElementById('pgoal').innerHTML = '<strong>Goal:</strong> ' + _global_esc(u.goal || 'Not set') + (u.gw ? ' · Target: ' + _global_esc(u.gw) + ' lbs' : '');
