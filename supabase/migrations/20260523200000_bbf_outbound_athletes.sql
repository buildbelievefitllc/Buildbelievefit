-- ═══════════════════════════════════════════════════════════════════════
-- bbf_outbound_athletes · Multi-Agent Marketing Engine backbone.
-- ───────────────────────────────────────────────────────────────────────
-- Outbound athlete cold-outreach pipeline. Distinct from public.bbf_leads
-- which feeds the inbound site lead-capture funnel · we never merge them.
--
-- State machine:
--   raw          (ingested)
--   → analyzed   (pitch generated by Gemini)
--   → contacted  (sent via Resend, message_id stored)
--   → replied    (athlete replied, intent triaged)
--   → converted  (manual flip after CEO closes)
--   → bounced    (intent=not_interested OR delivery bounce)
--   → unsubscribed (one-click unsub via /api/v1/marketing/unsubscribe)
-- ═══════════════════════════════════════════════════════════════════════
...
create index if not exists idx_bbf_outbound_athletes_status
  on public.bbf_outbound_athletes (status);
create index if not exists idx_bbf_outbound_athletes_intent
  on public.bbf_outbound_athletes (intent)
  where intent is not null;
create index if not exists idx_bbf_outbound_athletes_unsubscribe_token
  on public.bbf_outbound_athletes (unsubscribe_token)
...
-- Service-role only · PII + outreach state. Clients never touch this.
