-- ═══════════════════════════════════════════════════════════════════════════
-- BBF SUPABASE — BASELINE MIGRATION (Phase 3 P1)
-- ═══════════════════════════════════════════════════════════════════════════
-- Captures the state of the bbf-lab project (ihclbceghxpuawymlvgi) as of
-- 2026-04-29. Mirrors api/supabase-schema-actual.sql and is the canonical
-- starting point for the Supabase CLI migration history.
--
-- Timestamp 20260101000000 is intentionally pre-dated to ensure this
-- baseline orders BEFORE the existing in-flight migration record:
--   20260429054308_phase2_hotfix_uid_column
-- which is already registered with Supabase. After applying this baseline
-- via `supabase migration repair --status applied 20260101000000`, both
-- migrations will be marked applied and Supabase has a complete history.
--
-- IDEMPOTENCY: All statements use IF NOT EXISTS / OR REPLACE / DROP-then-
-- CREATE for policies and triggers. Re-running this migration on an already-
-- migrated database is a no-op.
--
-- See supabase/README.md for the install + link + repair workflow.
-- ═══════════════════════════════════════════════════════════════════════════
