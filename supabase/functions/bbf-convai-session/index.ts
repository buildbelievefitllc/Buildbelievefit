...
// Keys are the exact ElevenLabs {{dynamic_variable}} names the agent prompt binds to:
...
async function buildDynamicVariables(url: string, key: string, userId: string, uid: string | null, mode: string, locale: string, sessionId: string) {
  ...
  try {
    // Real display name (bbf_users.name) beats the uid slug for what the agent says aloud.
    const r = await fetch(
      `${url}/rest/v1/bbf_users?id=eq.${encodeURIComponent(userId)}&select=name&limit=1`,
      { headers: pgHeaders(key) },
    );
    const rows: any[] = r.ok ? await r.json().catch(() => []) : [];
    const name = String(rows[0]?.name || '').trim();
    if (name) vars.client_name = name;
  } catch { /* name is best-effort context */ }
  try {
